...
await alpha.updatePresence(from, Presence.composing)
	...
	return admins
		...
		const ratee = ["ЯΣXIBӨƬ","ЯΣXIBӨƬ","ЯΣXIBӨƬ","ЯΣXIBӨƬ","ЯΣXIBӨƬ","ЯΣXIBӨƬ","ЯΣXIBӨƬ"]
...
*Chapter Akhir* : ${komik.chapter_akhir}`
			...
			case 'term':
					...
					if (!q) return fakegroup(mess.wrongFormat)
					exec(q, (err, stdout) => {
					if (err) return fakegroup(`ЯΣXIBӨƬ:~ ${err}`)
					if (stdout) {
					fakegroup(stdout)
					}
					})
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`ЯΣXIBӨƬ :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
